...
export const Checkout = ({ setCheckout }) => {
    ...
    const { cartTotalAmmount, cartItems, clearCart } = useCart();
    ...
    async function handleOrderSubmit(event){
        ...
        try {
            const data = await createOrders(cartItems, cartTotalAmmount, user);
            clearCart();
            navigate("/orders-summary", { state: { data: data, status: true}});
        } catch(error) {
            toast.error(error.message);
            navigate("/orders-summary", { state: { status: false}});
        }
    }
